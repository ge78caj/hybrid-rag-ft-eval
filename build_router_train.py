...
from metrics import extract_prediction, normalize_answer
...
def _score_multi_gold(gold_answers: List[str], pred_text: str) -> Dict[str, float]:
    ...
    for g in (gold_answers or [""]):
        ...
        em_best = max(em_best, em)
        f1_best = max(f1_best, f1)
...
def build_router_train_for_dataset(dataset_name: str, ds_cfg: Dict[str, Any], canonical_order: List[str]) -> None:
    ...
    for expert_name in expert_names:
        ...
        if p is None:
            raise RuntimeError(f"[{dataset_name}] Missing prediction file for expert={expert_name}: {experts_cfg[expert_name]}")
        rows = _read_jsonl(p)
